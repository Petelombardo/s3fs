...
static string cache_expiration;
static string gzip_folders;
...
calc_signature(string method, string content_type, string date, curl_slist* headers, string resource) {
	...
	int count = 0;
	if (headers != 0) {
                cout << "[calc_signature] Dumping Headers" << endl;
		do {
...
get_contentEncoding(string to) {
	cout << "Getting Content Encoding for folder " << to << "\n";
...
//		cout << "." << extension << " Extension Found" << endl;
	...
	if (lastdir == "jsz" or extension == "jgz" or extension == "cssz") {
		return "gzip";
	}
      // Begin Gzip folder command-line support
        // Gzip support only works on folders that match the base of the path.  We do not match wholly contained substrings.
        std::vector<std::string> folders = split(gzip_folders, ':');
        for( std::vector<string>::iterator i = folders.begin(); i != folders.end(); ++i) { 
                std::string folder = *i;
                cout << "Checking " << folder << "\n";
                std::string::size_type found = to.find(folder);
                if (found != std::string::npos && found == 0 && ( extension == "js" or extension == "css" )) {
                        cout << "Found: " << std::string::npos << "::" << found << "\n";
                        cout << "This folder is specified to be gzipped.  Setting the content encoding accordingly.\n";
                        return "gzip";
                }
        }
        // End Gzip folder command-line support
...
put_headers(const char* path, headers_t meta) {
  ...
  string ContentEncoding = meta["Content-Encoding"];
  string CacheControl = meta["Cache-Control"];
  ...
  cout << "[put_headers] copying[path=" << path << "]" << endl;
...
//	curl_easy_setopt(curl, CURLOPT_ENCODING, "gzip"); // gzip enabled
...
//	cout << "Setting content encoding to: " << contentEncoding << endl;
        if (cache_expiration != "") {
                headers.append("Cache-Control: max-age="+cache_expiration);
        }
...
s3fs_rename(const char *from, const char *to) {
    ...
    cout << "[Rename] Setting content encoding to: " << contentEncoding << endl;
    if (contentEncoding == "gzip") {
	    meta["Content-Encoding"]="gzip";
    }
    meta["Content-Type"]=contentType;
    meta["x-amz-metadata-directive"] = "REPLACE";
    cout << "[Rename] Content-Type: "<< meta["Content-Type"] << endl;
...
my_fuse_opt_proc(void *data, const char *arg, int key, struct fuse_args *outargs) {
	...
	if (key == FUSE_OPT_KEY_OPT) {
		...
		if (strstr(arg, "cache_expiration=") != 0) {
			cache_expiration = strchr(arg, '=') +1;
			return 0;
		}
                if (strstr(arg, "gzip_folders=") != 0) {
                        gzip_folders = strchr(arg, '=') +1;
                        return 0;
                }
